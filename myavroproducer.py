...
from models import RecordBasicModel
...
def defined_producer() -> SerializingProducer:
    ...
    avro_value_serializer = AvroSerializer(schema_registry_client=schema_reg_client,
                                           schema_str=schemas.record_value_v1,
                                           to_dict=lambda rec, ctx: rec.dict())
...
@app.post('/produce/data', status_code=201, response_model=list[RecordBasicModel])
def record_producer(command: NumOfRecordsCommand):
    lo_records: list[RecordBasicModel] = []
    fake = Faker()
    record: RecordBasicModel
    producer = defined_producer()

    for _ in range(command.records):
        record = RecordBasicModel(ssn=fake.ssn(),
                                  name=fake.name(),
                                  dob=f"{fake.date_of_birth().day}-"
                                      f"{fake.date_of_birth().month}-"
                                      f"{fake.date_of_birth().year}")
        lo_records.append(record)
